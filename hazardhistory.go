...
type HazardHistoryListResponse struct {
	...
	Alarms []string `json:"alarms" api:"required"`
	...
	AlarmValues []float64 `json:"alarmValues" api:"required"`
	...
	DataMode HazardHistoryListResponseDataMode `json:"dataMode" api:"required"`
	// The detect time, in ISO 8601 UTC format, with millisecond precision.
	DetectTime time.Time `json:"detectTime" api:"required" format:"date-time"`
	// The type of hazard (Chemical, Biological, Radiological, Nuclear) detect
	// associated with this record.
	DetectType string `json:"detectType" api:"required"`
	// Source of the data.
...
type HazardHistoryListParams struct {
	...
	DetectTime time.Time `query:"detectTime" api:"required" format:"date-time" json:"-"`
