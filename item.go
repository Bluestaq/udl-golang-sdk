...
type ItemListResponse struct {
	...
	JSON struct {
		...
		Type                  resp.Field
		ID                    resp.Field
		AccSysKeys            resp.Field
		AccSysNotes           resp.Field
		AccSystem             resp.Field
		AccSysValues          resp.Field
		Airdrop               resp.Field
		AltDataFormat         resp.Field
		CargoType             resp.Field
		CenterlineOffset      resp.Field
		Cg                    resp.Field
		CommodityCode         resp.Field
		CommoditySys          resp.Field
		Container             resp.Field
		CreatedAt             resp.Field
		CreatedBy             resp.Field
		Departure             resp.Field
		Destination           resp.Field
		DvCode                resp.Field
		Fs                    resp.Field
		HazCodes              resp.Field
		Height                resp.Field
		IDAirLoadPlan         resp.Field
		ItemContains          resp.Field
		Keys                  resp.Field
		LastArrDate           resp.Field
		Length                resp.Field
		Moment                resp.Field
		Name                  resp.Field
		NetExpWt              resp.Field
		Notes                 resp.Field
		NumPalletPos          resp.Field
		Origin                resp.Field
		OrigNetwork           resp.Field
		ProductCode           resp.Field
		ProductSys            resp.Field
		ReceivingBranch       resp.Field
		ReceivingUnit         resp.Field
...
func (f ItemNewParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemUpdateParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemListParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemCountParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemGetParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemTupleParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemUnvalidatedPublishParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f ItemUnvalidatedPublishParamsBody) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
