...
// parameter.
func (r *PoiService) Get(ctx context.Context, id string, query PoiGetParams, opts ...option.RequestOption) (res *PoiFull, err error) {
	opts = append(r.Options[:], opts...)
	if id == "" {
...
// hours ago.
func (r *PoiService) Tuple(ctx context.Context, query PoiTupleParams, opts ...option.RequestOption) (res *[]PoiFull, err error) {
	opts = append(r.Options[:], opts...)
	path := "udl/poi/tuple"
	err = requestconfig.ExecuteNewRequest(ctx, http.MethodGet, path, query, &res, opts...)
	return
