...
type SensorListResponseSensorcharacteristic struct {
	...
	AcceptableSatIDFormats []string `json:"acceptableSatIdFormats"`
	...
	MaxDataTransmissionRate float64 `json:"maxDataTransmissionRate"`
	...
	MaxNumTracks int64 `json:"maxNumTracks"`
	...
	NumSearchPatterns int64 `json:"numSearchPatterns"`
	...
	ObsPerTrack int64 `json:"obsPerTrack"`
	// Time between observations in a track in seconds.
	ObsSpacing float64 `json:"obsSpacing"`
	...
	// VOICE-ONLY, etc.).
	TaskingChannels []string `json:"taskingChannels"`
	...
	JSON struct {
		...
		Source                          respjson.Field
		ID                              respjson.Field
		AcceptableSatIDFormats          respjson.Field
		...
		MaxDataTransmissionRate         respjson.Field
		MaxDeviationAngle               respjson.Field
		MaxIntegrationTime              respjson.Field
		MaxNumTracks                    respjson.Field
		...
		NumSearchPatterns               respjson.Field
		NumWaveforms                    respjson.Field
		ObsPerTrack                     respjson.Field
		ObsSpacing                      respjson.Field
		...
		TaskingChannels                 respjson.Field
...
type SensorGetResponseSensorcharacteristic struct {
	...
	SearchPatternPoints []float64 `json:"searchPatternPoints"`
	// Time for a sensor to settle after slewing to a new boresight location and before
	// collecting observations, in seconds.
	SettleTime float64 `json:"settleTime"`
	// Radar signal processing losses, in decibels.
	...
	JSON struct {
		...
		RightGeoBeltLimit               respjson.Field
		RunMeanCodes                    respjson.Field
		SearchPatternPoints             respjson.Field
		SettleTime                      respjson.Field
		SignalProcessingLoss            respjson.Field
		SiteCode                        respjson.Field
		SolarExclAngle                  respjson.Field
...
type SensorNewParamsSensorcharacteristic struct {
	...
	// Maximum data transmission rate, in gigabytes per second.
	MaxDataTransmissionRate param.Opt[float64] `json:"maxDataTransmissionRate,omitzero"`
	// Max deviation angle of the sensor in degrees.
	...
	// the capability to form multiple beams. For all sensors, it includes capability
	// to distinguish multiple objects within a single beam or field-of-view.
	MaxNumTracks param.Opt[int64] `json:"maxNumTracks,omitzero"`
	// Maximum observable sensor range, in kilometers.
	...
	// Number of search patterns.
	NumSearchPatterns param.Opt[int64] `json:"numSearchPatterns,omitzero"`
	// The number of waveforms characteristics provided in this record. If provided,
	// the numWaveforms value indicates the number of elements in each of the
	// waveformPulseWidths, waveformBandWidths, waveformMinRanges, waveformMaxRanges,
	// and waveformLoopGains arrays.
	...
	// Standard number of observations within a track.
	ObsPerTrack param.Opt[int64] `json:"obsPerTrack,omitzero"`
	// Time between observations in a track in seconds.
	ObsSpacing param.Opt[float64] `json:"obsSpacing,omitzero"`
	// Fraction of incident light transmitted to an optical sensor focal plane array.
	// The value is given as a fraction of 1, not as a percent.
	...
	// Time for a sensor to settle after slewing to a new boresight location and before
	// collecting observations, in seconds.
	SettleTime param.Opt[float64] `json:"settleTime,omitzero"`
	// Radar signal processing losses, in decibels.
	...
	// transmission (e.g. 5-DIGIT, 9-DIGIT, ALPHA-5, INTERNATIONAL DESIGNATOR, etc.).
	// These formats are not necessarily applicable to UDL sensor tasking.
	AcceptableSatIDFormats []string `json:"acceptableSatIdFormats,omitzero"`
	// Array of measurement range(s) where radar samples must fall to be acceptable. If
	// this field is populated, the associated beam(s) must be provided in the
	// beamOrder field.
	...
	// Array of the data formats supported by the sensor associated with this record.
	DataFormats []string `json:"dataFormats,omitzero"`
	// Array of time delay(s) for pulses from a radar beam to get to the first range
	// gate, in nanoseconds. If this field is populated, the associated beam(s) must be
	// provided in the beamOrder field.
	...
	// degrees). The initial time is always 0, and subsequent times are in seconds
	// since the first point.
	SearchPatternPoints []float64 `json:"searchPatternPoints,omitzero"`
	// Array of the number(s) of Doppler spectra used to process measurements from
	// radar. Spectral averaging involves combining multiple Doppler spectra acquired
	// to obtain a more accurate and representative spectrum. If this field is
	// populated, the associated beam(s) must be provided in the beamOrder field.
	...
	// Array of available tasking channels (e.g. AD-HOC, CTL, DAILY, EVENT-ONLY,
	// VOICE-ONLY, etc.).
	TaskingChannels []string `json:"taskingChannels,omitzero"`
	// Array of temporal median filter code(s) of a radar beam. Temporal median
	// filtering is a noise-reducing algorithm which involves replacing each data point
	// with the median value of a window of neighboring points over time. If this field
	// is populated, the associated beam(s) must be provided in the beamOrder field.
