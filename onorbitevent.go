...
func (r *OnorbiteventService) Queryhelp(ctx context.Context, opts ...option.RequestOption) (err error) {
	opts = append(r.Options[:], opts...)
	opts = append([]option.RequestOption{option.WithHeader("Accept", "")}, opts...)
...
type OnorbiteventListResponse struct {
	...
	JSON struct {
		...
		DataMode                      resp.Field
		EventTime                     resp.Field
		Source                        resp.Field
		ID                            resp.Field
		AchievedFlightPhase           resp.Field
		AgeAtEvent                    resp.Field
		CapabilityLoss                resp.Field
		CapabilityLossNotes           resp.Field
		CapacityLoss                  resp.Field
		ConsequentialEquipmentFailure resp.Field
		...
		EquipmentAtFault              resp.Field
		EquipmentCausingLossNotes     resp.Field
		EquipmentPartAtFault          resp.Field
		EquipmentTypeAtFault          resp.Field
		EventResult                   resp.Field
		EventTimeNotes                resp.Field
		EventType                     resp.Field
		GeoPosition                   resp.Field
		IDOnOrbit                     resp.Field
		Inclined                      resp.Field
		Injured                       resp.Field
		InsuranceCarriedNotes         resp.Field
		InsuranceLoss                 resp.Field
		InsuranceLossNotes            resp.Field
		Killed                        resp.Field
		LesseeOrgID                   resp.Field
		LifeLost                      resp.Field
		NetAmount                     resp.Field
		ObjectStatus                  resp.Field
		OccurrenceFlightPhase         resp.Field
		OfficialLossDate              resp.Field
		OperatedOnBehalfOfOrgID       resp.Field
		OperatorOrgID                 resp.Field
		Origin                        resp.Field
		OrigNetwork                   resp.Field
		OrigObjectID                  resp.Field
		OwnerOrgID                    resp.Field
		PlaneNumber                   resp.Field
		PlaneSlot                     resp.Field
		PositionStatus                resp.Field
		Remarks                       resp.Field
		SatellitePosition             resp.Field
		SatNo                         resp.Field
		StageAtFault                  resp.Field
		ThirdPartyInsuranceLoss       resp.Field
		UnderlyingCause               resp.Field
		UntilTime                     resp.Field
...
func (f OnorbiteventNewParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f OnorbiteventUpdateParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f OnorbiteventListParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f OnorbiteventCountParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f OnorbiteventGetParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f OnorbiteventTupleParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
