...
func (f VideoHistoryListParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f VideoHistoryCountParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
