...
<a href="https://pkg.go.dev/github.com/stainless-sdks/unifieddatalibrary-go"><img src="https://pkg.go.dev/badge/github.com/stainless-sdks/unifieddatalibrary-go.svg" alt="Go Reference"></a>
...
```go
import (
	"github.com/stainless-sdks/unifieddatalibrary-go" // imported as unifieddatalibrary
)
```
...
```sh
go get -u 'github.com/stainless-sdks/unifieddatalibrary-go@v0.0.1-alpha.0'
```
...
func main() {
	client := unifieddatalibrary.NewClient(
		option.WithPassword("My Password"), // defaults to os.LookupEnv("UDL_AUTH_PASSWORD")
		option.WithUsername("My Username"), // defaults to os.LookupEnv("UDL_AUTH_USERNAME")
	)
...
The unifieddatalibrary library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `unifieddatalibrary.String(string)`, `unifieddatalibrary.Int(int64)`, etc.

Any `param.Opt[T]`, map, slice, struct or string enum uses the
tag <code>\`json:"...,omitzero"\`</code>. Its zero value is considered omitted.

```go
params := unifieddatalibrary.ExampleParams{
	ID:   "id_xxx",                         // required property
	Name: unifieddatalibrary.String("..."), // optional property

	Point: unifieddatalibrary.Point{
		X: 0,                         // required field will serialize as 0
		Y: unifieddatalibrary.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: unifieddatalibrary.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.OverrideObj[unifieddatalibrary.FooParams](12)
```
...
client.Elsets.Current.List(context.TODO(), ...,
	// Override the header
...
`*unifieddatalibrary.Error`. This contains the `StatusCode`, `*http.Request`, and
...
```go
_, err := client.Elsets.Current.List(context.TODO(), unifieddatalibrary.ElsetCurrentListParams{})
if err != nil {
	...
	panic(err.Error()) // GET "/udl/elset/current": 400 Bad Request { ... }
}
```
...
client.Elsets.Current.List(
	ctx,
	unifieddatalibrary.ElsetCurrentListParams{},
...
We also provide a helper `unifieddatalibrary.File(reader io.Reader, filename string, contentType string)`
which can be used to wrap any `io.Reader` with the appropriate file name and content type.

```go
// A file from the file system
file, err := os.Open("/path/to/file")
unifieddatalibrary.AnalyticImageryUnvalidatedPublishParams{
	File: file,
}
...
unifieddatalibrary.AnalyticImageryUnvalidatedPublishParams{
	File: unifieddatalibrary.File(strings.NewReader(`{"hello": "foo"}`), "file.go", "application/json"),
}
```
...
page, err := client.Elsets.Current.List(
	context.TODO(),
	unifieddatalibrary.ElsetCurrentListParams{},
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/unifieddatalibrary-go/issues) with questions, bugs, or suggestions.
