...
// AnalyticImageryHistoryService contains methods and other services that help with
// interacting with the unifieddatalibrary API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewAnalyticImageryHistoryService] method instead.
type AnalyticImageryHistoryService struct {
	Options []option.RequestOption
}

// NewAnalyticImageryHistoryService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewAnalyticImageryHistoryService(opts ...option.RequestOption) (r AnalyticImageryHistoryService) {
	r = AnalyticImageryHistoryService{}
	r.Options = opts
	return
}
...
// URLQuery serializes [AnalyticImageryHistoryListParams]'s query parameters as
// `url.Values`.
func (r AnalyticImageryHistoryListParams) URLQuery() (v url.Values, err error) {
