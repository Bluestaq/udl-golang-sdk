...
func TestEmitterGeolocationNewWithOptionalParams(t *testing.T) {
	...
	err := client.EmitterGeolocation.New(context.TODO(), unifieddatalibrary.EmitterGeolocationNewParams{
		...
		AvgPrf:                unifieddatalibrary.Float(624.57),
		AvgPri:                unifieddatalibrary.Float(1601.1),
		AvgPw:                 unifieddatalibrary.Float(400.2),
		...
		MaxPrf:                unifieddatalibrary.Float(624.96),
		MaxPri:                unifieddatalibrary.Float(1602.1),
		MaxPw:                 unifieddatalibrary.Float(400.3),
		MinFreq:               unifieddatalibrary.Float(1575.42),
		MinPrf:                unifieddatalibrary.Float(624.18),
		MinPri:                unifieddatalibrary.Float(1600.1),
		MinPw:                 unifieddatalibrary.Float(400.1),
...
func TestEmitterGeolocationQueryHelp(t *testing.T) {
	...
	_, err := client.EmitterGeolocation.QueryHelp(context.TODO())
	if err != nil {
		var apierr *unifieddatalibrary.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
