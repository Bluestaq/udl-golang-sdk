...
// the [NewConjunctionHistoryService] method instead.
type ConjunctionHistoryService struct {
	Options []option.RequestOption
}

// NewConjunctionHistoryService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewConjunctionHistoryService(opts ...option.RequestOption) (r ConjunctionHistoryService) {
	r = ConjunctionHistoryService{}
	r.Options = opts
	return
}
...
func (r *ConjunctionHistoryService) Count(ctx context.Context, query ConjunctionHistoryCountParams, opts ...option.RequestOption) (res *string, err error) {
	opts = slices.Concat(r.Options, opts)
	opts = append([]option.RequestOption{option.WithHeader("Accept", "text/plain")}, opts...)
	path := "udl/conjunction/history/count"
...
// `url.Values`.
func (r ConjunctionHistoryCountParams) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
