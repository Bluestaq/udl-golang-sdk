...
type AnalyticImageryService struct {
	...
	History AnalyticImageryHistoryService
}
...
func NewAnalyticImageryService(opts ...option.RequestOption) (r AnalyticImageryService) {
	...
	r.Options = opts
	r.History = NewAnalyticImageryHistoryService(opts...)
	return
}
...
type AnalyticImageryAbridged struct {
	...
	DataMode AnalyticImageryAbridgedDataMode `json:"dataMode" api:"required"`
	...
	ImageType string `json:"imageType" api:"required"`
	// The message time of this image record, in ISO8601 UTC format with millisecond
	// precision.
	MsgTime time.Time `json:"msgTime" api:"required" format:"date-time"`
	// Source of the data.
