...
func TestGlobalAtmosphericModelQueryHelp(t *testing.T) {
	...
	_, err := client.GlobalAtmosphericModel.QueryHelp(context.TODO())
	if err != nil {
		var apierr *unifieddatalibrary.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestGlobalAtmosphericModelUnvalidatedPublishWithOptionalParams(t *testing.T) {
	...
	err := client.GlobalAtmosphericModel.UnvalidatedPublish(context.TODO(), unifieddatalibrary.GlobalAtmosphericModelUnvalidatedPublishParams{
		...
		Filename:              unifieddatalibrary.String("glotec_elecden.geojson"),
		Filesize:              unifieddatalibrary.Int(2097152),
		NumAlt:                unifieddatalibrary.Int(35),
		NumLat:                unifieddatalibrary.Int(72),
		NumLon:                unifieddatalibrary.Int(72),
		Origin:                unifieddatalibrary.String("THIRD_PARTY_DATASOURCE"),
		ReportTime:            unifieddatalibrary.Time(time.Now()),
		StartAlt:              unifieddatalibrary.Float(8553.163773),
		StartLat:              unifieddatalibrary.Float(88.75),
		StartLon:              unifieddatalibrary.Float(177.5),
		State:                 unifieddatalibrary.String("PREDICTED"),
		StepLat:               unifieddatalibrary.Float(2.5),
		StepLon:               unifieddatalibrary.Float(5.5),
	})
	if err != nil {
		var apierr *unifieddatalibrary.Error
		if errors.As(err, &apierr) {
