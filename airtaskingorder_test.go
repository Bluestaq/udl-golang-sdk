...
func TestAirTaskingOrderNewWithOptionalParams(t *testing.T) {
	...
	err := client.AirTaskingOrders.New(context.TODO(), unifieddatalibrary.AirTaskingOrderNewParams{
		BeginTs:               time.Now(),
		ClassificationMarking: "U",
		DataMode:              unifieddatalibrary.AirTaskingOrderNewParamsDataModeTest,
		...
		AcMsnTasking: []unifieddatalibrary.AirTaskingOrderNewParamsAcMsnTasking{{
			CountryCode:    "US",
			TaskedService:  "A",
			UnitDesignator: "AMPHIB5DIV",
			AcMsnLocSeg: []unifieddatalibrary.AirTaskingOrderNewParamsAcMsnTaskingAcMsnLocSeg{{
				StartTime:    time.Now(),
			...
			IndAcTasking: []unifieddatalibrary.AirTaskingOrderNewParamsAcMsnTaskingIndAcTasking{{
				AcftType:        "F35A",
		...
		GenText: []unifieddatalibrary.AirTaskingOrderNewParamsGenText{{
		...
		NavalFltOps: []unifieddatalibrary.AirTaskingOrderNewParamsNavalFltOp{{
			ShipName:           "USS WASP",
...
func TestAirTaskingOrderGetWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AirTaskingOrders.Get(
		context.TODO(),
		"id",
		unifieddatalibrary.AirTaskingOrderGetParams{
...
func TestAirTaskingOrderCountWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AirTaskingOrders.Count(context.TODO(), unifieddatalibrary.AirTaskingOrderCountParams{
		FirstResult: unifieddatalibrary.Int(0),
		MaxResults:  unifieddatalibrary.Int(0),
	})
	if err != nil {
...
func TestAirTaskingOrderQueryHelp(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	err := client.AirTaskingOrders.QueryHelp(context.TODO())
	if err != nil {
		var apierr *unifieddatalibrary.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestAirTaskingOrderTupleWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AirTaskingOrders.Tuple(context.TODO(), unifieddatalibrary.AirTaskingOrderTupleParams{
		Columns:     "columns",
		FirstResult: unifieddatalibrary.Int(0),
		MaxResults:  unifieddatalibrary.Int(0),
	})
