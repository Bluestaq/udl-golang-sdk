...
// EoObservationHistoryService contains methods and other services that help with
// interacting with the unifieddatalibrary API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewEoObservationHistoryService] method instead.
type EoObservationHistoryService struct {
	Options []option.RequestOption
}

// NewEoObservationHistoryService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewEoObservationHistoryService(opts ...option.RequestOption) (r EoObservationHistoryService) {
	r = EoObservationHistoryService{}
	r.Options = opts
	return
}
...
func (r *EoObservationHistoryService) List(ctx context.Context, query EoObservationHistoryListParams, opts ...option.RequestOption) (res *pagination.OffsetPage[EoObservationFull], err error) {
...
// (/udl/&lt;datatype&gt;/queryhelp) for more details on valid/required query
// parameter information.
func (r *EoObservationHistoryService) ListAutoPaging(ctx context.Context, query EoObservationHistoryListParams, opts ...option.RequestOption) *pagination.OffsetPageAutoPager[EoObservationFull] {
...
// (/udl/&lt;datatype&gt;/queryhelp) for more details on valid/required query
// parameter information.
func (r *EoObservationHistoryService) Aodr(ctx context.Context, query EoObservationHistoryAodrParams, opts ...option.RequestOption) (err error) {
	opts = append(r.Options[:], opts...)
...
func (r *EoObservationHistoryService) Count(ctx context.Context, query EoObservationHistoryCountParams, opts ...option.RequestOption) (res *string, err error) {
	opts = append(r.Options[:], opts...)
...
type EoObservationFull struct {
	...
	JSON struct {
		...
		EoobservationDetails  resp.Field
...
type EoObservationFullEoobservationDetails struct {
	...
	JSON struct {
		...
		AcalCrValX                           resp.Field
		AcalCrValY                           resp.Field
		AcalNumStars                         resp.Field
		BackgroundSignal                     resp.Field
		BackgroundSignalUnc                  resp.Field
		BinningHoriz                         resp.Field
		BinningVert                          resp.Field
		CcdObjPosX                           resp.Field
		CcdObjPosY                           resp.Field
		CcdObjWidth                          resp.Field
		CcdTemp                              resp.Field
		CentroidColumn                       resp.Field
		CentroidRow                          resp.Field
		ClassificationMarking                resp.Field
		ColorCoeffs                          resp.Field
		ColumnVariance                       resp.Field
		CreatedAt                            resp.Field
		CreatedBy                            resp.Field
		CurrentNeutralDensityFilterNum       resp.Field
		CurrentSpectralFilterNum             resp.Field
		DataMode                             resp.Field
		DeclinationCov                       resp.Field
		DistFromStreakCenter                 resp.Field
		Does                                 resp.Field
		ExtinctionCoeffs                     resp.Field
		ExtinctionCoeffsUnc                  resp.Field
		Gain                                 resp.Field
		IDEoObservation                      resp.Field
		Ifov                                 resp.Field
		MagInstrumental                      resp.Field
		MagInstrumentalUnc                   resp.Field
		NeutralDensityFilterNames            resp.Field
		NeutralDensityFilterTransmissions    resp.Field
		NeutralDensityFilterTransmissionsUnc resp.Field
		NumCatalogStars                      resp.Field
		NumCorrelatedStars                   resp.Field
		NumDetectedStars                     resp.Field
		NumNeutralDensityFilters             resp.Field
		NumSpectralFilters                   resp.Field
		ObjSunRange                          resp.Field
		ObTime                               resp.Field
		OpticalCrossSection                  resp.Field
		OpticalCrossSectionUnc               resp.Field
		PcalNumStars                         resp.Field
		PeakApertureCount                    resp.Field
		PeakBackgroundCount                  resp.Field
		PhaseAngBisect                       resp.Field
		PixelArrayHeight                     resp.Field
		PixelArrayWidth                      resp.Field
		PixelMax                             resp.Field
		PixelMin                             resp.Field
		PredictedAzimuth                     resp.Field
		PredictedDeclination                 resp.Field
		PredictedDeclinationUnc              resp.Field
		PredictedElevation                   resp.Field
		PredictedRa                          resp.Field
		PredictedRaUnc                       resp.Field
		RaCov                                resp.Field
		RaDeclinationCov                     resp.Field
		RowColCov                            resp.Field
		RowVariance                          resp.Field
		SnrEst                               resp.Field
		SolarDiskFrac                        resp.Field
		Source                               resp.Field
		SpectralFilters                      resp.Field
		SpectralFilterSolarMag               resp.Field
		SpectralZmfl                         resp.Field
		SunAzimuth                           resp.Field
		SunElevation                         resp.Field
		SunStatePosX                         resp.Field
		SunStatePosY                         resp.Field
		SunStatePosZ                         resp.Field
		SunStateVelX                         resp.Field
		SunStateVelY                         resp.Field
		SunStateVelZ                         resp.Field
		SurfBrightness                       resp.Field
		SurfBrightnessUnc                    resp.Field
		TimesUnc                             resp.Field
		Toes                                 resp.Field
		ZeroPoints                           resp.Field
		ZeroPointsUnc                        resp.Field
		ExtraFields                          map[string]resp.Field
		raw                                  string
	} `json:"-"`
}
...
type EoObservationHistoryListParams struct {
	// Ob detection time in ISO 8601 UTC, up to microsecond precision. Consumers should
	// contact the provider for details on their obTime specifications.
...
func (f EoObservationHistoryListParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }

// URLQuery serializes [EoObservationHistoryListParams]'s query parameters as
// `url.Values`.
func (r EoObservationHistoryListParams) URLQuery() (v url.Values, err error) {
...
type EoObservationHistoryAodrParams struct {
	// Ob detection time in ISO 8601 UTC, up to microsecond precision. Consumers should
	// contact the provider for details on their obTime specifications.
	// (YYYY-MM-DDTHH:MM:SS.ssssssZ)
	ObTime time.Time `query:"obTime,required" format:"date-time" json:"-"`
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f EoObservationHistoryAodrParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }

// URLQuery serializes [EoObservationHistoryAodrParams]'s query parameters as
// `url.Values`.
func (r EoObservationHistoryAodrParams) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
...
type EoObservationHistoryCountParams struct {
	// Ob detection time in ISO 8601 UTC, up to microsecond precision. Consumers should
	// contact the provider for details on their obTime specifications.
	// (YYYY-MM-DDTHH:MM:SS.ssssssZ)
	ObTime      time.Time        `query:"obTime,required" format:"date-time" json:"-"`
...
// IsPresent returns true if the field's value is not omitted and not the JSON
// "null". To check if this field is omitted, use [param.IsOmitted].
func (f EoObservationHistoryCountParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }

// URLQuery serializes [EoObservationHistoryCountParams]'s query parameters as
// `url.Values`.
func (r EoObservationHistoryCountParams) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
