...
// the [NewCollectResponseTupleService] method instead.
type CollectResponseTupleService struct {
	Options []option.RequestOption
}

// NewCollectResponseTupleService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewCollectResponseTupleService(opts ...option.RequestOption) (r CollectResponseTupleService) {
	r = CollectResponseTupleService{}
	r.Options = opts
	return
}
...
// hours would return the satNo and period of elsets with an epoch greater than 5
// hours ago.
func (r *CollectResponseTupleService) ListAutoPaging(ctx context.Context, query CollectResponseTupleListParams, opts ...option.RequestOption) *pagination.OffsetPageAutoPager[shared.CollectResponseFull] {
	return pagination.NewOffsetPageAutoPager(r.List(ctx, query, opts...))
}

type CollectResponseTupleListParams struct {
